...
router.post(
  ...
  [
    ...
    body("password", "Password must be 5 charcter").isLength({ min: 5 }),
  ],
  async (req, res) => {
   let success = false
    ...
    try {
      ...
      if (user) {
        return res
          .status(400)
          .json({success, error: "Invalid email this email already exist" });
      }
      ...
      res.status(500).send(success, "Enternal server error");
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!passwordCompare) {
        success = false
        return res.status(400).json({success, error: "Enter your Correct Password" });
      }
...
// Route 3: Get logging user detail: POST "api/auth/getuser" Login required
